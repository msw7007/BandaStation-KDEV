...
type MemorySet = z.infer<typeof memorySetSchema>;
...
export function chatMemorySet(payload: string): void {
  let memorySet: MemorySet;
  try {
    const parsed = JSON.parse(payload);
    memorySet = memorySetSchema.parse(parsed);
  } catch (err) {
    console.error(err);
    return;
  }

  chatRenderer.replaceMemoryBackedChat(memorySet.messages || []);
  saveChatToStorage();
}
