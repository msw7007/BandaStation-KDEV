...
const MEMORY_BACKED_MESSAGE_TYPES = new Set([
  MESSAGE_TYPE_COMBAT,
  MESSAGE_TYPE_ENTERTAINMENT,
  MESSAGE_TYPE_INFO,
  MESSAGE_TYPE_LOCALCHAT,
...
function isMemoryBackedMessage(message) {
  return !!message.cyMemoryTracked || !!message.cyMemoryId || MEMORY_BACKED_MESSAGE_TYPES.has(message.type);
}
...
class ChatRenderer {
  ...
  processBatch(
    ...
    for (const payload of batch) {
      ...
      message.node = node;
      if (message.cyMemoryTracked || message.cyMemoryId) {
        node.setAttribute('data-cy-memory-tracked', 'true');
        if (message.cyMemoryId) {
          node.setAttribute('data-cy-memory-id', String(message.cyMemoryId));
        }
      }
  ...
  replaceMemoryBackedChat(batch: Array<Record<string, unknown>>) {
    if (!this.isReady()) {
      this.queue = [
        ...this.queue.filter((message) => !isMemoryBackedMessage(message)),
        ...batch,
      ];
      return;
    }

    const keptMessages = this.messages.filter(
      (message) => !isMemoryBackedMessage(message),
    );

    const removedMessages = this.messages.filter((message) =>
      isMemoryBackedMessage(message),
    );
    ...
    logger.log(`Replaced memory-backed chat`);
  }
