...
/obj/machinery/vending/proc/proceed_payment(obj/item/card/id/paying_id_card, mob/living/mob_paying, datum/data/vending_product/product_to_vend, price_to_use, discountless)
	...
	if(vendor_corporation_id && vendor_corporation_id != "starlight")
		SScyberpunk_corporations.record_cyberpunk_corporate_activity(vendor_corporation_id, "sales", 1, max(0, round(price_to_use * 0.01)), "corporate vending purchase: [product_to_vend.name]")
	if(SScyberpunk_corporations.cyberpunk_corporation_has_edict("starlight", "starlight_self_statistics"))
		SScyberpunk_corporations.record_cyberpunk_corporate_activity("starlight", "market", 1, 0, "purchase telemetry: [product_to_vend.name]")
	if(vendor_corporation_id && SScyberpunk_corporations.cyberpunk_corporation_has_edict(vendor_corporation_id, "[vendor_corporation_id]_supply_cert"))
		SScyberpunk_corporations.record_cyberpunk_corporate_activity(vendor_corporation_id, "supply", 1, max(0, round(price_to_use * 0.01)), "certified vending supply: [product_to_vend.name]")
	//CYBERPUNK BUILD - rebuild and delete before release
	return TRUE
