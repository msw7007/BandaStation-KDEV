...
/datum/reagent/consumable/pwr_game/expose_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume)
	. = ..()
	if(exposed_mob?.get_cy_skill_level(/datum/cy_skill/charisma/style) >= CY_SKILL_LEVEL_MASTER && (methods & INGEST) && !HAS_TRAIT(exposed_mob, TRAIT_GAMERGOD))
...
/datum/reagent/consumable/pwr_game/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolization_ratio)
	...
	if(SPT_PROB(5, seconds_per_tick))
		affected_mob.award_cy_raw_skill_experience(/datum/cy_skill/charisma/style, 5)
