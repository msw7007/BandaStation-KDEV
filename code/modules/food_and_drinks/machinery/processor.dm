...
/obj/machinery/processor/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	...
	if(recipe && user.transferItemToLoc(tool, src))
		user.visible_message(
			span_notice("[capitalize(user.declent_ru(NOMINATIVE))] помещает [tool.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)]."),
			span_notice("Вы помещаете [tool.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)]."),
		)
