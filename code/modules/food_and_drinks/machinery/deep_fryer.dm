...
/obj/machinery/deepfryer/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	if(istype(tool, /obj/item/reagent_containers/applicator/pill))
		if(!reagents.total_volume)
			to_chat(user, span_warning("Не в чем растворять [tool.declent_ru(ACCUSATIVE)]!"))
			return ITEM_INTERACT_BLOCKING
		user.visible_message(span_notice("[capitalize(user.declent_ru(NOMINATIVE))] бросает [tool.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)]."), span_notice("Вы растворяете [tool.declent_ru(ACCUSATIVE)] в [declent_ru(PREPOSITIONAL)]."))
...
/obj/machinery/deepfryer/process(seconds_per_tick)
	...
	if(cook_time >= FRYING_TIME_PERFECT && !frying_fried)
		...
		visible_message(span_warning("[capitalize(declent_ru(NOMINATIVE))] источает едкий запах!"), ignored_mobs = anosmia_havers)
