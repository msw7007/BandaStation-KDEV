...
/proc/above_neck(zone)
	var/list/zones = list(BODY_ZONE_HEAD, BODY_ZONE_PRECISE_MOUTH, BODY_ZONE_PRECISE_NOSE, BODY_ZONE_PRECISE_EYES, BODY_ZONE_PRECISE_EARS, BODY_ZONE_PRECISE_NECK)
	if(zones.Find(zone))
		return TRUE
	else
		return FALSE
...
/proc/offer_control(mob/M)
	if(isdead(M))
		to_chat(usr, "Вы не можете дать призракам контроль над призраком. Они уже призраки.")
		return FALSE
