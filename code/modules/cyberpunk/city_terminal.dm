...
/obj/machinery/computer/cy_city_terminal/proc/show_city_status(mob/user)
	...
	if(istype(living_user))
		to_chat(user, span_notice("Vitals: [living_user.get_cy_secondary_indicator_summary().Join("; ")]"))
		...
		var/controlled_count = length(living_user.get_cy_controlled_items_in_zone())
		if(controlled_count)
