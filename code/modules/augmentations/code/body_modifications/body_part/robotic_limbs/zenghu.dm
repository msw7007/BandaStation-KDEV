// Zenghu Surplus (хламовый уровень)

/obj/item/bodypart/arm/left/robot/surplus/zenghu
	should_draw_greyscale = FALSE
	icon_state = "robotic_l_arm"
	icon_static = 'code/modules/augmentations/icons/zenghu_augments.dmi'
	icon = 'code/modules/augmentations/icons/zenghu_augments.dmi'

/obj/item/bodypart/arm/right/robot/surplus/zenghu
	should_draw_greyscale = FALSE
	icon_state = "robotic_r_arm"
	icon_static = 'code/modules/augmentations/icons/zenghu_augments.dmi'
	icon = 'code/modules/augmentations/icons/zenghu_augments.dmi'

/obj/item/bodypart/leg/left/robot/surplus/zenghu
	should_draw_greyscale = FALSE
	icon_state = "robotic_l_leg"
	icon_static = 'code/modules/augmentations/icons/zenghu_augments.dmi'
	icon = 'code/modules/augmentations/icons/zenghu_augments.dmi'

/obj/item/bodypart/leg/right/robot/surplus/zenghu
	should_draw_greyscale = FALSE
	icon_state = "robotic_r_leg"
	icon_static = 'code/modules/augmentations/icons/zenghu_augments.dmi'
	icon = 'code/modules/augmentations/icons/zenghu_augments.dmi'

// Zenghu Normal (стандартный уровень, cyborg)

/obj/item/bodypart/arm/left/robot/zenghu
	should_draw_greyscale = FALSE
	icon_state = "robotic_l_arm"
	icon_static = 'code/modules/augmentations/icons/zenghu_augments.dmi'
	icon = 'code/modules/augmentations/icons/zenghu_augments.dmi'

/obj/item/bodypart/arm/right/robot/zenghu
	should_draw_greyscale = FALSE
	icon_state = "robotic_r_arm"
	icon_static = 'code/modules/augmentations/icons/zenghu_augments.dmi'
	icon = 'code/modules/augmentations/icons/zenghu_augments.dmi'

/obj/item/bodypart/leg/left/robot/zenghu
	should_draw_greyscale = FALSE
	icon_state = "robotic_l_leg"
	icon_static = 'code/modules/augmentations/icons/zenghu_augments.dmi'
	icon = 'code/modules/augmentations/icons/zenghu_augments.dmi'

/obj/item/bodypart/leg/right/robot/zenghu
	should_draw_greyscale = FALSE
	icon_state = "robotic_r_leg"
	icon_static = 'code/modules/augmentations/icons/zenghu_augments.dmi'
	icon = 'code/modules/augmentations/icons/zenghu_augments.dmi'

// Zenghu Advanced (продвинутый уровень)

/obj/item/bodypart/arm/left/robot/advanced/zenghu
	should_draw_greyscale = FALSE
	icon_state = "robotic_l_arm"
	is_emissive = FALSE
	icon_static = 'code/modules/augmentations/icons/zenghu_augments.dmi'
	icon = 'code/modules/augmentations/icons/zenghu_augments.dmi'

/obj/item/bodypart/arm/right/robot/advanced/zenghu
	should_draw_greyscale = FALSE
	icon_state = "robotic_r_arm"
	is_emissive = FALSE
	icon_static = 'code/modules/augmentations/icons/zenghu_augments.dmi'
	icon = 'code/modules/augmentations/icons/zenghu_augments.dmi'

/obj/item/bodypart/leg/left/robot/advanced/zenghu
	should_draw_greyscale = FALSE
	icon_state = "robotic_l_leg"
	is_emissive = FALSE
	icon_static = 'code/modules/augmentations/icons/zenghu_augments.dmi'
	icon = 'code/modules/augmentations/icons/zenghu_augments.dmi'

/obj/item/bodypart/leg/right/robot/advanced/zenghu
	should_draw_greyscale = FALSE
	icon_state = "robotic_r_leg"
	is_emissive = FALSE
	icon_static = 'code/modules/augmentations/icons/zenghu_augments.dmi'
	icon = 'code/modules/augmentations/icons/zenghu_augments.dmi'
