...
/datum/unit_test/mob_chains/Run()
	...
	TEST_ASSERT_EQUAL(centipede_head.get_brute_loss(), 0, "Centipede head took damage which should have been passed to its tail.")
	TEST_ASSERT_EQUAL(centipede_tail.get_brute_loss(), test_damage, "Centipede tail did not take damage which should have originated from its head.")
	...
	TEST_ASSERT_EQUAL(victim.get_brute_loss(), expected_damage * 3, "Centipede failed to do damage with all of its segments.")
