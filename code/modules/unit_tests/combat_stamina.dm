...
/datum/unit_test/stamcrit/Run()
	...
	var/stamloss_to_reach_critical_health_threshold = tider.maxHealth
	tider.adjust_stamina_loss(stamloss_to_reach_critical_health_threshold - 1)
	TEST_ASSERT(!tider.has_status_effect(/datum/status_effect/incapacitating/stamcrit), "Stamcrit should not be applied at [stamloss_to_reach_critical_health_threshold - 1] stamina damage")
	tider.adjust_stamina_loss(1)
	TEST_ASSERT(tider.has_status_effect(/datum/status_effect/incapacitating/stamcrit), "Stamcrit should be applied at [stamloss_to_reach_critical_health_threshold] stamina damage")
