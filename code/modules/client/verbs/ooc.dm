...
/datum/codex/proc/get_codex_entries()
	...
	codex_entries = list(
			...
			"items" = list(
				list(
					...
					"sections" = list(
						list(
							"title" = "Что это",
							"body" = "- **Киберпространство** - цифровой слой станции, который строится в масштабе **1:1** по объектам физического мира.\n- Узлы собирают сетевые объекты одной зоны: двери, камеры, свет, консоли, APC, тревоги, серверы, терминалы, турели, ботов и импланты.\n- Следы с подписью **TRACE** - быстрые якоря к отдельным объектам вроде дверей, камер и света.\n- Живые носители нейроимпланта отображаются как зеленые нейронные слепки.\n\n**Эффект:** киберпространство позволяет взламывать инфраструктуру, извлекать net-data, атаковать ICE, использовать демонов и взаимодействовать с объектами без физического доступа к ним."
						),
						list(
							"title" = "Аватар",
							"body" = "- Вход создает цифровой аватар рядом с ближайшим сетевым объектом.\n- Тело расслабляется и падает, пока сознание находится в сети.\n- Аватар может двигаться по киберпространству, атаковать узлы, открывать соединения и возвращаться к телу.\n- Если нет открытого соединения с узлом, слишком дальний отход от безопасной точки вызывает перегрев и может повредить мозг.\n- Пока сознание в сети, тело в холодном помещении или рядом с заправленной льдом neural ice bath медленно снижает сетевой перегрев.\n\n**Эффект:** сеть дает удаленный доступ, но длинная незакрепленная проекция опасна для тела."
						),
						list(
							"title" = "Видимость и тревога",
							"body" = "- Провал ICE-последовательности или шумное подключение может поднять тревогу.\n- Камеры и нейроинтерфейсы рядом с целью усиливают обнаружение.\n- Навык взлома снижает заметность следа.\n- При тревоге появляется красный ping между атакующим и целью; сила сигнала зависит от сенсоров и навыка.\n\n**Эффект:** взлом рядом с камерами и владельцами нейроимплантов рискованнее, чем атака по изолированному объекту."
						),
					),
				),
				list(
					...
					"sections" = list(
						list(
							"title" = "Нейроимплант",
							"body" = "- Для обычного входа нужен рабочий нейроинтерфейс и живой мозг.\n- Команда **Enter Cyberspace** открывает проекцию в локальный слой.\n- Команда **Exit Cyberspace** возвращает сознание, если аватар достаточно близко к телу или якорю.\n- Возврат блокируется, если аватар ушел слишком далеко от допустимой точки выхода.\n\n**Эффект:** нейроимплант является штатным входом в сеть и основным каналом обратной связи."
						),
						list(
							"title" = "Криптоключи",
							"body" = "- Объекты и нейроинтерфейсы могут иметь криптографический ключ.\n- Если у пользователя есть подходящий ключ, объектный ICE можно обойти как легальный доступ.\n- Ключи могут попадать в интерфейс через награды Завесы, демонов и сетевую работу.\n\n**Эффект:** ключ не ломает ICE, а открывает соответствующий endpoint без атаки."
						),
						list(
							"title" = "Терминалы",
							"body" = "- Кибертерминалы и серверные объекты считаются сильными сетевыми целями.\n- Через терминалы можно получить доступ к программам, демонам, декомпиляции и работе с дисками.\n- Серверные цели обычно имеют больше объектного ICE и больше net-data.\n\n**Эффект:** терминалы - удобный вход в глубокую сетевую игру, но они заметнее и лучше защищены."
						),
					),
				),
				list(
					"id" = "network_nodes",
					"title" = "Узлы, ICE и данные",
					"sections" = list(
						list(
							"title" = "Узлы",
							"body" = "- Узел - группа сетевых объектов, собранная по зоне и близости.\n- Узел имеет общий ICE и список объектов внутри.\n- Отдельные TRACE-объекты дополнительно имеют собственный объектный ICE.\n- После подключения к узлу можно смотреть список объектов и выполнять доступные функции.\n\n**Эффект:** общий ICE защищает вход в узел, а объектный ICE защищает конкретные двери, камеры, свет, серверы и устройства."
						),
						list(
							"title" = "ICE",
							"body" = "- ICE состоит из параметров таймера, размера поля, длины последовательности и резерва защиты.\n- Взлом открывает мини-игру последовательностей.\n- Ошибки повышают шум и могут вызвать тревогу.\n- Урон по ICE снижает резерв; при нуле защита считается пробитой.\n\n**Эффект:** сильный ICE дольше держит объект закрытым и делает ошибочный взлом заметнее."
						),
						list(
							"title" = "Net-data",
							"body" = "- Net-data - цифровой ресурс, получаемый из объектов и пассивного присутствия в сети.\n- Двери, камеры и свет дают малые объемы данных.\n- Терминалы, серверы и важные машины дают больше.\n- Данные объекта извлекаются один раз после доступа, ключа или пробитого объектного ICE.\n\n**Эффект:** net-data тратится на сетевые возможности, разработку и работу с демонами."
						),
					),
				),
				list(
					...
					"sections" = list(
						list(
							"title" = "App Cracker",
							"body" = "- Программа App Cracker сканирует узлы, показывает DNS-метки, объекты, ICE, доступ и функции.\n- `ls` показывает найденные узлы, `ls -objs` - объекты подключенного узла.\n- `ping` проверяет цель, `select` выбирает объект, `crack` запускает взлом.\n- Команды функций работают только по объектам, к которым есть доступ.\n\n**Эффект:** App Cracker - основной текстовый инструмент для точечной сетевой работы."
						),
						list(
							"title" = "Функции",
							"body" = "- Двери можно открывать и закрывать.\n- Камеры можно глушить, свет - переключать.\n- APC, тревоги, консоли, устройства и интерфейсы имеют собственные endpoint-функции.\n- Если объектный ICE закрыт, функция недоступна до ключа или взлома.\n\n**Эффект:** контроль объекта всегда проверяет доступ именно к выбранному объекту, а не только к узлу."
						),
						list(
							"title" = "Стены",
							"body" = "- Киберстены могут замедлять или блокировать проход.\n- Незавершенная стена замедляет аватар, завершенная блокирует движение.\n- Стены ломаются демонами или прямым взломом кликом по стене.\n- Навык взлома ускоряет ручную атаку и повышает урон по стене.\n\n**Эффект:** стены создают маршрутные барьеры, но не являются абсолютной защитой."
						),
					),
				),
				list(
					...
					"sections" = list(
						list(
							"title" = "Сущность демона",
							"body" = "- Демон - сетевой исполняемый эффект: урон, перегрев, EMP, движение, стена, криптоключ, бафф, дебафф и другие действия.\n- Демоны имеют память, стоимость, производителя, силу, время подготовки, частоту и cooldown.\n- Спецэффекты вроде mass, spread, jump, repeat, stealth и EMP меняют способ применения.\n\n**Эффект:** демон - модульная способность сети, а не обычное оружие."
						),
						list(
							"title" = "Цели",
							"body" = "- Демоны могут работать по физическим целям, аватарам, энграммам, узлам, TRACE-объектам и стенам.\n- По TRACE-объекту защитные и атакующие демоны бьют объектный ICE.\n- По обычному узлу они работают с общим ICE узла.\n- Стеновые демоны строят или ломают киберстены.\n\n**Эффект:** выбор цели определяет, повреждается узел целиком или конкретный объект внутри него."
						),
						list(
							"title" = "Корпорации",
							"body" = "- Производитель демона влияет на синергию с нейроимплантом пользователя.\n- Корпоративные эдикты могут усиливать мощность, настройку и циклы отдельных производителей.\n- Совпадение корпоративной экосистемы повышает эффективность сетевой сборки.\n\n**Эффект:** корпоративная принадлежность важна не только для лора, но и для чисел."
						),
					),
				),
				list(
					"id" = "network_imprints",
					"title" = "Слепки и энграммы",
					"sections" = list(
						list(
							"title" = "Нейронный слепок",
							"body" = "- Носитель рабочего нейроимпланта отображается в сети как зеленый слепок.\n- Слепок имеет ICE нейроинтерфейса цели.\n- После пробития ICE можно кратко смотреть через чувства цели.\n- В боевом режиме пробитый слепок позволяет вызвать сбой конкретного киберимпланта; если подходящего импланта нет, цель получает перегрев нейроинтерфейса.\n\n**Эффект:** слепок - способ атаковать и наблюдать носителей нейроимпланта через сеть."
						),
						list(
							"title" = "Энграмма",
							"body" = "- Энграмма - цифровое состояние сознания, привязанное к чипу или энграмматору.\n- Якорь можно принять обратно, если энграмма находится рядом.\n- После переноса действует блокировка повторного перехода.\n- Если якорь уничтожен до освобождения, энграмма получает смертельную обратную связь.\n\n**Эффект:** энграмма дает независимое сетевое существование, но до освобождения полностью зависит от якоря."
						),
						list(
							"title" = "Свободная энграмма",
							"body" = "- Энграмма, проведшая 5 минут в Завесе, может освободиться от якоря.\n- Вне Завесы свободная энграмма сначала получает штрафы к движению и проверкам.\n- Каждые 30 минут в Завесе она проходит стадию стабилизации: сначала штрафы уменьшаются, затем исчезают, затем демоны вне Завесы становятся сильнее.\n- В Завесе свободная энграмма постепенно восстанавливает целостность аватара.\n\n**Эффект:** свободная энграмма становится самостоятельной, но требует времени в Завесе для полной силы."
						),
					),
				),
				list(
					...
					"sections" = list(
						list(
							"title" = "Опасная зона",
							"body" = "- Завеса - отдельная область киберпространства за пределами обычной сети.\n- Чем дальше аватар от сети, тем выше шанс ответа Завесы.\n- Ответ Завесы создает альтернатив, которые охотятся на аватар.\n- У аватара в Завесе есть отдельная целостность; при ее обнулении мозг получает тяжелую обратную связь.\n\n**Эффект:** Завеса нужна для редких наград и освобождения энграммы, но является самым опасным сетевым местом."
						),
						list(
							"title" = "Хранилища",
							"body" = "- В Завесе могут появляться сетевые хранилища и старые data vault.\n- Хранилище можно вытянуть в физический мир и открыть дедигитайзером.\n- Старые vault охраняются и требуют боя в киберпространстве.\n- Награда может включать данные, ключи и редкие цифровые объекты.\n\n**Эффект:** глубокая сеть дает лучший лут, но требует подготовки, защиты и пути назад."
						),
					),
				),
			),
			...
			"items" = list(
				list(
					...
					"sections" = list(
						list(
							"title" = "Сеть питания",
							"body" = "- Основа питания использует tgStation-систему: источники энергии, кабели, powernet, APC и потребление машин.\n- Внутренние помещения обычно питаются через APC своей Area.\n- Уличные, наружные и отдельные механизмы могут требовать прямой кабельной связи с внешней сетью или источником.\n- Источниками могут быть генераторы, городская инфраструктура, закупка энергии у правительства или корпоративная доставка энергии.\n\n**Эффект:** если механизм не получает питание через APC, ищите кабель, источник и связь с powernet."
						),
						list(
							"title" = "Машины и зарядки",
							"body" = "- Машины используют обычные проверки питания, каналы потребления и `use_energy()`.\n- Зарядные станции, батареи, генераторы и модульные улучшения работают через существующую систему машин.\n- Модули могут менять потребление, скорость зарядки и генерацию.\n- Электрика отвечает за питание, проводку, зарядки, генераторы и EMP-связанные задачи.\n\n**Эффект:** энергетика остается общей для города, машин и оборудования, а не отдельной ручной симуляцией."
						),
					),
				),
				list(
					"id" = "city_lightweight_atmos",
					"title" = "Воздух и атмосфера",
					"sections" = list(
						list(
							"title" = "Area-воздух",
							"body" = "- Полная станционная LINDA-атмосфера не используется как основная модель города.\n- Обычный кислород и CO2 хранятся в Area, а не как постоянная смесь на каждом turf.\n- Открытая Area считается бесконечно чистой и восполняет воздух.\n- Закрытая Area может постепенно терять кислород от дыхания и получать CO2.\n\n**Эффект:** для дыхания важна зона, в которой находится персонаж, а не скрытая газовая смесь каждого пола."
						),
						list(
							"title" = "Дыхание",
							"body" = "- Легкие берут кислород из Area.\n- При нехватке кислорода персонаж начинает задыхаться.\n- CO2 и токсичное качество воздуха могут наносить вред.\n- Маски, фильтры, внутренние баллоны и защиты от дыма продолжают влиять на дыхание.\n\n**Эффект:** закрытые помещения, подземные зоны и опасные среды имеют простой и быстрый учет воздуха."
						),
						list(
							"title" = "Трубы и установки",
							"body" = "- Трубы больше не означают полноценную комнатную LINDA-смесь.\n- Они работают как связь между канистрами, установками, вентами, скрубберами и совместимыми машинами.\n- Выпущенный газ преобразуется в временное облако, если попадает в среду.\n\n**Эффект:** трубная сеть обслуживает оборудование и выпуск газа, но не заставляет город считать атмосферу каждого тайла."
						),
					),
				),
				list(
					"id" = "city_clouds",
					"title" = "Дым, газ и облака",
					"sections" = list(
						list(
							"title" = "Облако",
							"body" = "- Облако - временный gas effect на turf: дым, токсин, plasma, огонь, холод, кислород, CO2, химический дым и другие эффекты.\n- Облако может нести реагенты, снижать видимость, действовать через дыхание или касание.\n- Облако постепенно рассеивается; малое количество исчезает.\n- Облака текут на соседние проходимые turf и реагируют на направление вентиляции.\n\n**Эффект:** необычный газ существует как локальное облако, а не как постоянное состояние комнаты."
						),
						list(
							"title" = "Химия и опасность",
							"body" = "- Химический дым теперь создает chemical cloud и переносит реагенты через lightweight atmos.\n- Реагенты могут попадать при дыхании или касании, если защита не блокирует эффект.\n- Пламя, холод, кислота, токсины и давление задаются типом облака.\n- Химия усиливает работу с дымом, реагентами и опасными облаками.\n\n**Эффект:** гранаты, реакции и машины дыма используют общий механизм облаков."
						),
						list(
							"title" = "Тревоги",
							"body" = "- Опасные облака могут запускать тревоги Area.\n- Вентиляция и скрубберы могут уводить или чистить scrubbable-облака.\n- Сильное облако может временно менять локальное чтение сканера и условия на своем turf.\n\n**Эффект:** опасность видна как локальная авария, но не превращает всю Area в постоянную газовую смесь."
						),
					),
				),
				list(
					...
					"sections" = list(
						list(
							"title" = "Где хранится",
							"body" = "- Базовая температура хранится у Area.\n- Локальная температура может быть у облака на turf.\n- Обычный turf без облака не является постоянным хранилищем температуры.\n- Сканеры и живые существа получают итоговую температуру из Area и облака на текущем turf.\n\n**Эффект:** система быстрая: зона задает фон, облако задает локальную аномалию."
						),
						list(
							"title" = "Эффекты",
							"body" = "- Жара и холод могут наносить HEAT/COLD-урон через обычные проверки среды.\n- Холод может помогать снижать перегрев имплантов, если соответствующая механика использует температуру окружения.\n- Жара может усиливать перегрев механизмов и имплантов, если они реагируют на среду.\n- Вентиляция постепенно возвращает Area к нормальной температуре, а облака могут ее сдвигать.\n\n**Эффект:** температура влияет на комфорт, тело и технику без возврата к полной turf-атмосфере."
						),
					),
				),
				list(
					"id" = "city_vents_filters",
					"title" = "Вентиляция и фильтры",
					"sections" = list(
						list(
							"title" = "Венты и скрубберы",
							"body" = "- Венты пополняют Area воздухом из открытой чистой среды или совместимой системы.\n- Скрубберы чистят CO2 Area и scrubbable-облака.\n- Работа зависит от питания, включения, направления, сварки, поломки и целостности.\n- Устройства могут засоряться и терять эффективность.\n\n**Эффект:** вентиляция управляет воздухом, облаками и температурой, но не включает полную LINDA-атмосферу комнат."
						),
						list(
							"title" = "Обслуживание",
							"body" = "- Засоренные фильтры можно обслуживать инструментом.\n- Строительство ускоряет работу с корпусами, вентиляцией, стенами и укреплениями.\n- Электрика важна для питания, проводки и энергетической части оборудования.\n- Изобретательство связано с модулями, батареями, зарядными устройствами и механизмами.\n\n**Эффект:** технические навыки влияют на эксплуатацию городской инфраструктуры."
						),
					),
				),
				list(
					"id" = "city_environment_skills",
					"title" = "Навыки среды",
					"sections" = list(
						list(
							"title" = "Выживание",
							"body" = "- Выживание снижает тяжесть вреда от опасной среды, если персонаж проходит соответствующие проверки.\n- Это влияет на удушье, токсичность и вредные облака, но не усиливает полезные облака.\n- Защита, маски и внутренний воздух остаются важнее навыка.\n\n**Эффект:** выживание помогает пережить плохую среду, но не заменяет оборудование."
						),
						list(
							"title" = "Профессии",
							"body" = "- Химия отвечает за газы, реагенты, дым и опасные облака.\n- Электрика отвечает за питание, проводку, зарядки, генераторы и EMP.\n- Строительство отвечает за корпуса, вентиляцию, стены и укрепления.\n- Изобретательство отвечает за модули, батареи, зарядные устройства и механизмы.\n\n**Эффект:** инфраструктура города завязана на разные специализации, а не на один универсальный инженерный навык."
						),
					),
				),
			),
		...
		list(
			...
			"items" = list(
				list(
					"id" = "city_roles_basics",
					"title" = "Общее",
					"sections" = list(
						list(
							"title" = "Что задает роль",
							"body" = "- Роль задает стартовое имущество, доступные стартовые задачи, стартовые бонусы и направление игры.\n- Роль не задает билд персонажа: характеристики, навыки, перки, черты и корпоративная привязка остаются отдельным выбором игрока.\n- Игрок может заранее подготовить очки и получить их из префов, а роль только задает стартовую рамку.\n\n**Эффект:** роль помогает войти в городскую жизнь, но не запирает персонажа в одном стиле игры."
						),
						list(
							"title" = "Жители",
							"body" = "- Житель - базовая городская роль для игроков, которые сами выбирают занятие.\n- Житель может работать, торговать, открывать бизнес, вступать в договоренности, выполнять услуги или просто быть частью жизни города.\n- Для NPC житель означает обычного городского персонажа, который поддерживает фон города и может получать простые городские задачи.\n\n**Эффект:** житель не обязан иметь отдельный сценарий от роли; его геймплей задает сам игрок."
						),
					),
				),
				list(
					"id" = "city_roles_civic",
					"title" = "Городские роли",
					"sections" = list(
						list(
							"title" = "Офицер",
							"body" = "- Офицер занимается патрулем, охраной важных лиц и зданий, разбором нелегальных ситуаций и доставкой задержанных.\n- На текущем этапе офицер получает отдельный полицейский skillchip для списка розыска и содержания.\n- НПЦ-полиция может доставлять задержанных в участок; активный розыск при доставке переводится в содержание.\n\n**Эффект:** офицер связан с городским порядком и полицейской инфраструктурой, но право и наказания остаются IC-вопросом."
						),
						list(
							"title" = "Член Совета",
							"body" = "- Член Совета - отдельная городская роль для работы с правительством и кризисными решениями.\n- Роль рассчитана на переговоры, сверку данных, работу с представителями корпораций и решения по чрезвычайному положению.\n- В городе для Совета предусмотрены защищенные зоны: гермоврата и терминал блокировки ближайших гермоврат.\n\n**Эффект:** член Совета получает политическую и защитную инфраструктуру, а не боевой билд."
						),
						list(
							"title" = "Наемник",
							"body" = "- Наемник работает через контракты: доставка, поиск, сопровождение, услуги, серые поручения и задания от бизнеса, корпораций или правительства.\n- Стартовая основа роли - особый телефон с предустановленным приложением контрактов.\n- Дополнительное оружие и снаряжение наемников не входят в этот блок.\n\n**Эффект:** роль сразу подключает игрока к контрактной работе без отдельной жесткой профессии."
						),
					),
				),
				list(
					"id" = "city_roles_corporate",
					"title" = "Корпоративные роли",
					"sections" = list(
						list(
							"title" = "Стажер, агент и специалист",
							"body" = "- Стажер - стартовая корпоративная роль без сильной власти, которую можно развить внутри корпорации.\n- Агент - боевое и оперативное направление: охрана, перехваты, саботаж, похищения и сопровождение задач.\n- Специалист - небоевое направление для развития корпорации, исследований, услуг, производства и контрактов.\n\n**Эффект:** корпоративная роль задает направление работы внутри корпорации, но билд игрок собирает отдельно."
						),
						list(
							"title" = "Корпоративные специализации",
							"body" = "- Benn тянет медицину, импланты, химию и биотехнологии.\n- Ryaznov тянет инженерку, строительство, добычу, ремонт и техническую экипировку.\n- Starlight тянет логистику, поставки, транспорт, маршруты и городскую доставку.\n- Представитель управляет местным подразделением и заинтересован в развитии корпорации и прибыли.\n\n**Эффект:** специализация объясняет, чем корпорат полезен корпорации, без создания отдельного билда вместо игрока."
						),
					),
				),
				list(
					"id" = "city_roles_limits",
					"title" = "Ограничения и NPC",
					"sections" = list(
						list(
							"title" = "Бонусы и ограничения",
							"body" = "- Роль может давать небольшой бонус к статам, навыку, стартовым деньгам, транспорту, доступу, экипировке или корпоративной связи.\n- Ограниченные роли выдаются по статистике игрока или решению администрации.\n- Ограничения роли не должны заменять билд и не должны автоматически решать IC-право.\n\n**Эффект:** роль дает стартовый вес, но не отменяет развитие персонажа и решения игроков."
						),
						list(
							"title" = "NPC на ролях",
							"body" = "- Важные городские роли могут временно занимать NPC, чтобы город не выглядел пустым.\n- NPC могут поддерживать патруль, охрану, доставку, ремонт, торговлю или нападения в пределах своей задачи.\n- Если игрок занимает роль, NPC-замещение для этой роли может быть удалено или не использоваться.\n\n**Эффект:** NPC закрывают пустоты города, но не заменяют живого игрока там, где игрок уже взял роль."
						),
						list(
							"title" = "Отложенные роли",
							"body" = "- Антагонисты, банды, мутанты, конструкты, рой и армия спасения остаются отдельной отложенной частью.\n- Их логика, цели и ограничения будут описываться после утверждения идей блока.\n\n**Эффект:** текущий Кодекс фиксирует городские, корпоративные и контрактные роли, не притворяясь, что антаги уже готовы."
						),
					),
				),
			),
		...
		list(
			...
			"items" = list(
				list(
					...
					"sections" = list(
						list(
							"title" = "Предприятие игрока",
							"body" = "- Бизнес - персистентное предприятие игрока в городе.\n- Создается через терминал ведения бизнеса и после развертывания привязывается к нейроинтерфейсу владельца.\n- Владелец управляет направлением, легальностью, складом, работниками, оплатой, счетом и доступами к терминалу.\n- Доступы можно выдавать другим персонажам, но сохранение и загрузка бизнеса не доступны для взлома ради защиты от грифа.\n\n**Правило:** сохранить бизнес можно сколько угодно раз, загрузить сохраненный бизнес можно только один раз за раунд."
						),
						list(
							"title" = "Сохранение",
							"body" = "- Сохранение фиксирует планировку, оборудование, мебель, склад, терминалы, автоматы, защитные системы и счет.\n- Загрузка восстанавливает сохраненное состояние в новом раунде.\n- Сохранение и загрузка считаются владельческими действиями, а не обычными правами доступа.\n\n**Эффект:** игрок может развивать устойчивое предприятие между раундами, не превращая сохранение в инструмент захвата чужого имущества."
						),
						list(
							"title" = "Легальный и нелегальный бизнес",
							"body" = "- Легальный бизнес зарегистрирован, платит налоги, виден в городском реестре бизнесов и может пользоваться наймом и контрактами.\n- Нелегальный бизнес существует без регистрации, не обязан занимать официальный лот и может работать хоть из квартиры или дома.\n- Нелегальный бизнес не использует системы официального найма и контрактов.\n- Главное различие - налоги, публичная известность и доступ к официальным системам города.\n\n**Эффект:** город поддерживает и витринные компании, и подпольную экономику."
						),
					),
				),
				list(
					"id" = "city_warehouse_logistics",
					"title" = "Склады и логистика",
					"sections" = list(
						list(
							"title" = "Склад бизнеса",
							"body" = "- Склад - выделенная часть помещения, откуда бизнес пополняет товары, ингредиенты и ресурсы.\n- Автоматы, протолаты, фабрикаторы и другие машины могут брать материалы из связанного склада.\n- Для склада нужна зона разгрузки 3x3, через которую проходят погрузка и разгрузка дронов или машин.\n- Законно отвести все бизнес-помещение под склад или выставку.\n\n**Эффект:** ресурсы не нужно хранить по ящикам во всех комнатах, если бизнес настроил складскую зону."
						),
						list(
							"title" = "Цены и связи",
							"body" = "- Владелец выставляет цену за единицу товара и может задавать скидку или надбавку для корпораций и гражданских поставок.\n- Через терминал настраивается автопополнение и предел излишков, которые можно отдавать по запросам.\n- При восстановлении бизнеса владелец выбирает, с какими складами он связан для закупки и отдачи.\n- Доступ к отдельным складам можно блокировать или запрашивать.\n\n**Эффект:** склад работает как торговая и производственная точка, а не только как комната хранения."
						),
						list(
							"title" = "Доставка",
							"body" = "- Автоматические запросы на пополнение сначала выбирают склады с низкой ценой товара.\n- Между складами летает АВИ - заранее подготовленная транспортная машина; посадка и взлет являются визуальной частью процесса.\n- В автоматы и терминалы товары доставляются дронами: один вид товара за рейс, но в любом количестве.\n- АВИ вызываются владельцем через терминал бизнеса или терминал доставки склада.\n\n**Эффект:** логистика связывает бизнесы, склады, автоматы и городские поставки в одну экономическую сеть."
						),
					),
				),
				list(
					"id" = "city_credits_taxes",
					"title" = "Кредиты, счета и налоги",
					"sections" = list(
						list(
							"title" = "Кредит",
							"body" = "- Кредит - универсальная валюта обмена.\n- Деньги хранятся на банковских счетах игроков, корпораций и бизнесов.\n- Счет - уникальная виртуальная запись, с которой списываются и на которую зачисляются средства.\n- Банковские данные считаются размещенными на правительственных серверах.\n\n**Эффект:** экономика города строится вокруг счетов, переводов, налогов, закупок и продаж."
						),
						list(
							"title" = "Доходы и рынок",
							"body" = "- Игроки и корпорации могут зарабатывать продажей товаров за пределы карты.\n- Средства можно выводить через закупки извне.\n- Биржевые игры зависят от показателей корпорации, чьи акции используются.\n\n**Эффект:** городская экономика связана не только с локальными покупками, но и с внешним рынком."
						),
						list(
							"title" = "Налоги",
							"body" = "- Легальный бизнес и корпорации платят налоги правительству.\n- Неуплата отображается в правительственных или корпоративных терминалах.\n- Игроки сами решают, взыскивать долг, договариваться или урегулировать вопрос иначе.\n\n**Эффект:** налоги являются поводом для политики, давления и сделок, а не только автоматическим числом."
						),
					),
				),
				list(
					"id" = "city_corporations",
					"title" = "Корпорации",
					"sections" = list(
						list(
							"title" = "Роль в городе",
							"body" = "- Корпорации - ядро городской экономики и крупные публичные силы.\n- Базовые участники системы: Benn, Ryaznov, Starlight и правительственный блок.\n- Корпорации управляют счетами, сотрудниками, ролями, зарплатами, доступами, сервисами, исследованиями и эдиктами.\n- Корпоративная работа ведется через терминалы и программу Ntos Corporations.\n\n**Эффект:** корпорация дает направление, инфраструктуру, бренд и экономические рычаги."
						),
						list(
							"title" = "Сотрудники и роли",
							"body" = "- Сотрудников можно вводить в корпорацию, назначать им роль, зарплату и доступы.\n- Роли задают направление работы и не требуют отдельной системы автонайма или должностных шаблонов.\n- Зарплатные ведомости и выплаты проходят через корпоративный счет.\n\n**Эффект:** управление персоналом остается привязанным к ролям и живым решениям игроков."
						),
						list(
							"title" = "Исследования и эдикты",
							"body" = "- Корпорации накапливают исследовательские данные и открывают технологии.\n- Эдикты меняют правила работы корпорации, усиливают сервисы или задают корпоративную политику.\n- У корпораций есть публичная и закрытая информация, а также производители и брендовые линейки.\n\n**Эффект:** корпорация развивается не только деньгами, но и технологическим и политическим весом."
						),
					),
				),
				list(
					"id" = "city_corporate_services",
					"title" = "Сервисы, агенты и бренды",
					"sections" = list(
						list(
							"title" = "Сервисы",
							"body" = "- Benn работает с медициной, телом, химией, дыханием, скрытностью и страховыми эффектами.\n- Ryaznov закрывает технические, инженерные, ремонтные, энергетические и защитные задачи.\n- Starlight специализируется на доставке, возврате, транспорте, маяках, маршрутах и логистическом влиянии.\n- Заявка на сервис оплачивается и либо выполняется автоматически, либо уходит в очередь.\n\n**Эффект:** корпорации продают не только предметы, но и городские услуги."
						),
						list(
							"title" = "Агенты",
							"body" = "- У корпораций есть пул агентов для выполнения сервисных задач.\n- Временные NPC-агенты могут появляться для подтверждения и сопровождения услуги.\n- Невыполненные заявки можно выставлять как публичные аутсорс-контракты.\n\n**Эффект:** сервисы можно отыгрывать через игроков, агентов и контрактную систему."
						),
						list(
							"title" = "Брендовые предметы",
							"body" = "- Benn: Lifeline analyzer, ClearBreath medipen, PulseGuard vial, PulseTab и ClearTab.\n- Ryaznov: Rivet welder, Torque wrench, Gridkey multitool и Prybar.\n- Starlight: Beacon lamp, Burst radio, Pathfinder multitool и Quickjack.\n- Брендовые наборы поддерживают стиль и специализацию корпорации.\n\n**Эффект:** корпорации узнаются в игре через услуги, оборудование и расходники."
						),
					),
				),
				list(
					...
					"sections" = list(
						list(
							"title" = "Финансовый надзор",
							"body" = "- Правительство ведет налоговый мониторинг корпораций, бизнесов и жилья.\n- В терминале видны долги, ставки, принудительные переводы и сбор задолженностей.\n- Правительство может управлять директивами, арендой жилья и экстренными голосованиями.\n\n**Эффект:** правительство является финансовым и административным центром города."
						),
						list(
							"title" = "Санкции",
							"body" = "- Корпорации могут получать штраф, аудит или приостановку.\n- Бизнесы могут получать штраф, аудит или отзыв легального статуса.\n- Санкции являются игровым инструментом давления и урегулирования, а не заменой отыгрыша.\n\n**Эффект:** нарушения становятся поводом для конфликта, переговоров и административных решений."
						),
					),
				),
				list(
					"id" = "city_map_status",
					"title" = "Карта и тестовый режим",
					"sections" = list(
						list(
							"title" = "Текущее состояние",
							"body" = "- Настоящая карта города будет собираться отдельно.\n- Код корпораций, бизнеса, правительства, терминалов, плат и дизайнов существует независимо от финального расположения на карте.\n- Для тестов оставлен временный IC-verb `(TEMP) Create Corporate Terminal`, который создает корпоративный или правительственный терминал на текущем тайле.\n\n**Эффект:** механики можно тестировать до релизной карты."
						),
						list(
							"title" = "Что останется для карты",
							"body" = "- На настоящей карте нужно выбрать места для правительственных и корпоративных терминалов.\n- Нужно расставить офисы, бизнес-лоты, склады, зоны разгрузки, вывески и доступы.\n- Временный verb должен быть удален перед релизом, когда появится настоящая карта и нормальная выдача терминалов.\n\n**Эффект:** незакрытая часть карты - это размещение и оформление, а не отсутствие базового кода."
						),
					),
				),
			),
		...
		list(
			...
			"items" = list(
				list(
					"id" = "cyberpunk_ai_civilians",
					"title" = "Гражданские NPC",
					"sections" = list(
						list(
							"title" = "Обычное поведение",
							"body" = "- Гражданские NPC используют стандартный tg AI controller: задачи, память и маршруты хранятся в blackboard, а действия выполняются через AI behaviors.\n- В спокойном состоянии они могут патрулировать район, выполнять простую работу, доставку, ремонт или ожидание по назначенной городской задаче.\n- Если рядом нет наблюдателей, часть задач может идти в фантомном режиме: NPC логически продолжает маршрут или работу без полного физического тика тела.\n\n**Эффект:** гражданские создают движение города, но не обязаны физически просчитываться каждую секунду вне зоны игроков."
						),
						list(
							"title" = "Реакция на события",
							"body" = "- Гражданский NPC реагирует на задачу, угрозу, повреждение, маршрут, груз и доступные возможности своего профиля.\n- При доставке он идет к источнику, подбирает груз, несет его получателю и при необходимости возвращается в точку возврата.\n- При ремонте он идет к цели и чинит ее, если профиль позволяет ремонт.\n- При исчезновении цели, груза или маршрута задача может быть провалена с причиной в blackboard.\n\n**Эффект:** поведение зависит не от скрытого сценария, а от текущего состояния задачи и возможностей NPC."
						),
						list(
							"title" = "Наблюдаемость",
							"body" = "- Если игрок, камера или другой наблюдатель может увидеть NPC, он должен быть физически представлен в актуальной фазе задачи.\n- При выходе из фантомного режима NPC появляется около своей расчетной позиции; если он нес груз, система пытается восстановить физическое состояние груза.\n- Cross-Z наблюдение учитывает соседние уровни у маршрута и целей, чтобы NPC не оставался невидимым там, где его можно заметить.\n\n**Эффект:** игрок не должен видеть пустое место там, где по расчету городской ИИ уже должен находиться."
						),
					),
				),
				list(
					"id" = "cyberpunk_ai_hostiles",
					"title" = "Враги и охрана",
					"sections" = list(
						list(
							"title" = "Угрозы",
							"body" = "- Враждебные, охранные и полицейские NPC используют threat blackboard: цель угрозы, уровень угрозы и профиль роли.\n- Боевой профиль разрешает реакцию на угрозы, помощь союзникам и передачу тревоги ближайшим боеспособным NPC.\n- Раненый NPC может сменить поведение: прекратить давление, отступить или запросить помощь, если его профиль и состояние это позволяют.\n\n**Эффект:** враги и охрана реагируют через общую систему AI, а не через отдельную глобальную боевую автоматику."
						),
						list(
							"title" = "Фракции и роли",
							"body" = "- Роль NPC задает вес задач: рабочие лучше подходят для ремонта и доставок, контракторы для контрактов, охрана и полиция для патруля и защиты.\n- Фракция может ограничивать, какой NPC примет задачу или чей приказ будет учитывать.\n- Охранные NPC могут получать задачу сопровождения или доставки задержанной цели в безопасную/служебную зону.\n\n**Эффект:** разные NPC могут использовать один механизм задач, но выбирать и исполнять их с разными приоритетами."
						),
						list(
							"title" = "Фантомное состояние",
							"body" = "- Вне наблюдения враг или охранник может продолжать простую задачу компактно: маршрут, возврат, ожидание или завершение работы.\n- Если NPC был ранен, фантомное состояние хранит здоровье и может ухудшаться со временем.\n- Уровень ИИ снижает шанс фантомного провала маршрута: слабый NPC чаще теряет задачу, сильный стабильнее доходит.\n\n**Эффект:** опасность не исчезает за пределами экрана, но и не требует полного физического просчета каждой скрытой сцены."
						),
					),
				),
				list(
					"id" = "cyberpunk_ai_delivery",
					"title" = "MULE и AVi",
					"sections" = list(
						list(
							"title" = "Общее назначение",
							"body" = "- **MULE** и **AVi** относятся к логистике Starlight и используются для доставки груза между точками.\n- Игрок указывает pickup, dropoff и список грузов; система проверяет валидность точек и запускает подходящий транспорт.\n- Стоимость Starlight-доставки зависит от типа транспорта и количества груза.\n\n**Эффект:** это сервисная доставка, а не ручное телепортирование предметов."
						),
						list(
							"title" = "MULE",
							"body" = "- MULE используется для наземной доставки.\n- Он требует, чтобы pickup и dropoff находились на одном Z-уровне.\n- Если маршрут или точки не подходят, заказ отклоняется и оплата должна быть возвращена логикой сервиса.\n\n**Эффект:** MULE дешевле и проще, но ограничен обычным наземным маршрутом."
						),
						list(
							"title" = "AVi",
							"body" = "- AVi используется для воздушной доставки и подходит для маршрутов, где наземный MULE неудобен.\n- Он создается на точке pickup и получает задачу доставки до dropoff через собственную логику транспорта.\n- AVi дороже, но лучше подходит для многоуровневого города и срочной логистики.\n\n**Эффект:** AVi закрывает воздушный слой доставки и помогает связывать районы, разделенные высотой или сложной геометрией."
						),
						list(
							"title" = "Связь с городским ИИ",
							"body" = "- Обычные NPC-курьеры и транспортная доставка используют разные исполнители, но одну городскую идею: источник, получатель, груз, состояние и результат.\n- NPC-курьер идет через AI controller и GOAP-задачу; MULE/AVi идут через транспортную логику.\n- Контрактная доставка может вызвать AI-курьера, если предмет подходит под условия контракта.\n\n**Эффект:** игрок выбирает между живым курьером, наземной машиной и воздушной доставкой по цене, доступности и риску."
						),
					),
				),
			),
