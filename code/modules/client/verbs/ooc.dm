			...
			"items" = list(
				list(
					...
					"sections" = list(
						list(
							"title" = "Мышь",
							"body" = "- **LMB** — базовое действие: взять, снять, открутить.\n- **RMB** — вторичное действие: дать, поставить, прикрутить.\n- **MMB** — специальное действие или активация выбранного демона.\n- **Ctrl+LMB** — схватить или потянуть цель.\n- **Ctrl+RMB** — указать на цель.\n- **Alt** — дополнительное или защитное действие.\n- **Alt+LMB** — дополнительное базовое действие.\n- **Alt+RMB** — дополнительное вторичное действие.\n- **Alt+MMB** — изучить груду.\n- **Shift+LMB** — рассмотреть человека или объект, посмотреть вниз на краю.\n- **Shift+RMB** — контекстное меню.\n- **Shift+MMB** — присмотреться вдаль, посмотреть наверх или прислушаться."
						),
						list(
							"title" = "Клавиши",
							"body" = "- **Shift** — бег.\n- **C** — бросить перенос.\n- **R** — приготовиться бросать.\n- **T** — говорить.\n- **Shift+T** — шепот.\n- **Y** — гарнитурное сообщение.\n- **L** — локальный OOC.\n- **O** — OOC.\n- **M** — действие.\n- **Q** — активация предмета в руке.\n- **Z** — уронить из рук.\n- **1-4 / F** — переключение боевого интента.\n- **Shift+Alt+H** — сдаться.\n- **TAB** — переключить, как или куда говорит персонаж.\n- **X** — сменить руку."
						),
						list(
							"title" = "Перетаскивание",
							"body" = "- **Перетянуть себя LMB** — сесть или пристегнуться.\n- **Перетянуть на себя RMB** — предложить понести человека на руках или плече.\n- **Перетянуть себя RMB** — попросить человека понести вас.\n- **Перетянуть себя MMB** — ERP.\n- **Перетянуть себя на тайл LMB** — вжаться в стену; в режиме скрытности повышает хамелеон.\n- **Перетянуть себя на стул, стол или кровать в режиме скрытности** — залезть под мебель и повысить хамелеон.\n- **В бою перетянуть человека на стул, стол или кровать** — швырнуть туда человека."
						),
					),
				),
				list(
			...
			"items" = list(
				list(
					...
					"sections" = list(
						list(
							"title" = "Пошатывание",
							"body" = "**Пошатывание** замедляет цель и делает ее уязвимее к последующим приемам. Пинок теперь пошатывает самого пинающего. Если цель уже пошатывается, успешный пинок сбивает ее с ног."
						),
						list(
							"title" = "Сбит с ног",
							"body" = "**Сбит с ног** временно переводит цель в лежачее положение. Часть источников дополнительно дает dazed-уязвимость к контролю."
						),
						list(
							"title" = "Пинок",
							"body" = "Пинок выполняется зажатым **RMB** в боевом режиме. При силе **10+** успешный пинок отбрасывает цель на две клетки. Пинок имеет собственное восстановление."
						),
						list(
							"title" = "Обход защиты",
							"body" = "- **Быстрый удар** через **Alt+LMB** готовится для обхода уклонения.\n- **Хитрый удар** через **Alt+RMB** готовится для обхода парирования.\n- Такие атаки требуют повышенной траты стамины."
						),
					),
				),
				list(
					"id" = "defense_effects",
					"title" = "Защитные состояния",
					"sections" = list(
						list(
							"title" = "Парирование",
							"body" = "**Парирование** защищает от части входящих атак. Против него используется хитрый удар."
						),
						list(
							"title" = "Уклонение",
							"body" = "**Уклонение** при успехе полностью игнорирует урон и эффект атаки. После успешного уворота персонаж мгновенно смещается на свободную соседнюю клетку рядом с атакующим: сначала пробует уйти в сторону от линии атаки, затем от противника, затем в любую доступную соседнюю клетку. Разворот персонажа при этом не меняется. Против уклонения используется быстрый удар."
						),
						list(
							"title" = "Стамина",
							"body" = "Атаки и специальные боевые действия расходуют стамину. Альтернативные атаки с обходом защиты расходуют больше стамины, чем обычный удар."
						),
					),
				),
			),
