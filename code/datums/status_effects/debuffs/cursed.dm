	...
	switch(curse_count)
		if(1) // basically your first is a "freebie" that will still require urgent medical attention and will leave you smoking forever but could be worse tbh
			if(ishuman(owner))
				...
				affecting.force_wound_upwards(/datum/wound/burn/flesh/critical/cursed_brand, wound_source = "curse of the slot machine")
...
/datum/status_effect/slot_machine_curse/proc/clear_curses()
	...
	if(!isnull(branded_hand))
		var/datum/wound/brand = branded_hand.get_wound_type(/datum/wound/burn/flesh/critical/cursed_brand)
		brand?.remove_wound()
