...
/datum/status_effect/incapacitating/sleeping/tick(seconds_between_ticks)
		...
		if(sleep_quality > 0)
			if(iscarbon(owner))
				...
				if(exercised && carbon_owner.mind)
					...
					carbon_owner.award_cy_raw_skill_experience(/datum/cy_skill/spirit/athletics, seconds_between_ticks * sleep_quality * SLEEP_QUALITY_WORKOUT_MULTIPLER)
	...
	if(prob(8) && owner.health > owner.critical_health_threshold)
		owner.emote("snore")
