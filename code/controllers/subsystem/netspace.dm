SUBSYSTEM_DEF(netspace)
	...
	var/list/net_reservations = list()
...
/datum/controller/subsystem/netspace/stat_entry(msg)
	...
	msg = "Avatars:[length(active_avatars)] Nodes:[length(registered_nodes)] Clusters:[length(net_clusters)] Ready:[ready_text]"
	return ..()
...
/datum/controller/subsystem/netspace/proc/get_cluster_key(area_type, net_x, net_y, net_z)
	// One visible node per area type per net-Z. net_x/net_y are intentionally ignored for compact area nodes.
	return "[net_z]:[area_type]"
...
/datum/controller/subsystem/netspace/proc/get_net_z_for_physical_z(physical_z)
	...
	var/datum/turf_reservation/reservation = SSmapping.request_turf_block_reservation(CY_NETSPACE_DEFAULT_WIDTH, CY_NETSPACE_DEFAULT_HEIGHT, 1)
	if(!reservation)
		return null

	net_reservations[key] = reservation
	var/net_z
...
/datum/controller/subsystem/netspace/proc/build_lobby_full_floor_queue()
	...
	var/list/seen_physical_z = list()
	for(var/atom/network_object as anything in default_object_registration_queue)
		if(!network_object?.z)
			continue
		seen_physical_z["[network_object.z]"] = TRUE
	for(var/z_key in seen_physical_z)
		var/net_z = get_net_z_for_physical_z(text2num(z_key))
		if(!net_z)
			continue
