...
/atom/proc/analyzer_act(mob/living/user, obj/item/tool)
	...
	var/analyze_time = 2 SECONDS * user.get_cyberpunk_analysis_time_multiplier()
	if(analyze_time > 0 && !do_after(user, analyze_time, target = src))
		return TRUE
	...
	var/new_type_chance = user.get_cyberpunk_skill_perk_bonus(SKILL_ANALYSIS, 4)
	var/technology_chance = user.get_cyberpunk_skill_perk_bonus(SKILL_ANALYSIS, 5)
	if(!user.try_cyberpunk_analysis_research_reward(src, new_type_chance, TRUE))
		user.try_cyberpunk_analysis_research_reward(src, technology_chance, FALSE)
	var/insight_chance = max(new_type_chance, technology_chance)
