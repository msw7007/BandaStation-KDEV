...
// MARK: Wall floor continuation
#define CYBERPUNK_WALL_CONTINUATION_MASKS 'modular_bandastation/aesthetics/walls/icons/interior_wall_underlay_masks.dmi'

/proc/cyberpunk_wall_continuation_underlay_icon(turf/center)
	...
	for(var/direction_text as anything in cardinal_masks)
		...
		if(!source_turf || iswallturf(source_turf) || !source_turf.icon)
			cache_key += "[direction_text]=skip;"
			continue

		var/source_icon = source_turf.icon
		var/source_icon_state = source_turf.icon_state || initial(source_turf.icon_state)
		var/source_dir = source_turf.dir
	...
	for(var/direction_text as anything in cardinal_masks)
		...
		source_part.AddAlphaMask(icon(CYBERPUNK_WALL_CONTINUATION_MASKS, cardinal_masks[direction_text]))
...
/turf/closed/wall/proc/clear_cyberpunk_neighbor_floor_underlay()
	if(!cyberpunk_neighbor_floor_underlay)
		return
	underlays -= cyberpunk_neighbor_floor_underlay
	cyberpunk_neighbor_floor_underlay = null

/turf/closed/wall/proc/update_cyberpunk_neighbor_floor_underlay()
	clear_cyberpunk_neighbor_floor_underlay()
	var/icon/continuation_icon = cyberpunk_wall_continuation_underlay_icon(src)
	if(!continuation_icon)
		return
	cyberpunk_neighbor_floor_underlay = mutable_appearance(continuation_icon, "", LOW_FLOOR_LAYER, src, FLOOR_PLANE)
	underlays += cyberpunk_neighbor_floor_underlay
...
#undef CYBERPUNK_WALL_CONTINUATION_MASKS
