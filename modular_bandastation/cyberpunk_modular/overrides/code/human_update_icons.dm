...
/proc/wear_digi_version(icon/base_icon, obj/item/item, key, greyscale_colors)
	...
	var/index = "[key]-[greyscale_colors]"
	var/static/list/digi_clothing_icons = list()
	var/icon/digi_clothing_icon = digi_clothing_icons[index]
	if(!isnull(digi_clothing_icon))
		return icon(digi_clothing_icon)

	var/icon/resulting_icon = item.generate_digitigrade_icons(base_icon, greyscale_colors)
	if(!resulting_icon)
	...
	digi_clothing_icons[index] = resulting_icon
	return icon(resulting_icon)

/mob/living/carbon/human/get_overlays_copy(list/unwantedLayers)
	var/list/out = new
...
/obj/item/get_worn_offsets(isinhands)
	. = list(0,0) //(px,py)
	if(isinhands)
		//Handle held offsets
		var/mob/holder = loc
...
/mob/get_item_offsets_for_index(i)
	switch(i)
		if(3) //odd = left hands
...
/mob/living/update_eyes(refresh = TRUE)
	update_code_fov()
	return
...
/mob/living/carbon/human/render_only_head()
	if(!dna?.species)
		return
