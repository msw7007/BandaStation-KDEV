// Cyberpunk modular override for code/datums/dna/dna.dm.
// Core file stays upstream-clean; this file loads late.
...
/datum/dna/add_mutation(mutation_to_add, list/sources)
	if(!islist(sources))
		if(!sources)
...
/datum/dna/remove_mutation(mutation_to_remove, list/sources)
	if(!islist(sources))
		if(!sources)
...
/datum/dna/check_mutation(mutation_type)
	return get_mutation(mutation_type)

/datum/dna/remove_all_mutations(sources = GLOB.standard_mutation_sources)
	remove_mutation_group(mutations, sources)
	scrambled = FALSE

/datum/dna/remove_mutation_group(list/group, sources = GLOB.standard_mutation_sources)
	if(!group)
		return
	for(var/mutation in group)
		remove_mutation(mutation, sources)

/datum/dna/generate_unique_identity()
	. = ""
 ...
 */
/datum/dna/generate_dna_blocks(list/mutation_blacklist)
...
/datum/dna/generate_unique_enzymes()
	. = ""
	if(istype(holder))
...
/datum/dna/set_uni_feature_block(blocknum, input)
...
/datum/dna/update_ui_block(blocktype)
	if(isnull(blocktype))
...
/datum/dna/update_uf_block(blocktype)
	if(!blocktype)
 ...
 */
/datum/dna/is_same_as(datum/dna/target_dna)
	if( \
 ...
 */
/datum/dna/initialize_dna(newblood_type = random_human_blood_type(), create_mutation_blocks = TRUE, randomize_features = TRUE)
	if(newblood_type)
...
/mob/set_species(datum/species/mrace, icon_update = 1)
	return
...
/datum/dna/get_mutation(mutation_path)
...
/datum/dna/activate_mutation(mutation) //note that this returns a boolean and not a new mob
	if(!mutation)
		return FALSE
...
/datum/dna/mutation_in_sequence(mutation)
	if(!mutation)
		return
	if(istype(mutation, /datum/mutation))
...
/mob/living/carbon/random_mutate(list/candidates)
	if(!has_dna())
...
/mob/living/carbon/get_random_mutation_path(quality = POSITIVE|NEGATIVE|MINOR_NEGATIVE, scrambled = TRUE, sequence = TRUE, list/excluded_mutations = list(/datum/mutation/race))
	if(!has_dna())
		return null
...
/mob/living/carbon/easy_random_mutate(quality = POSITIVE|NEGATIVE|MINOR_NEGATIVE, scrambled = TRUE, sequence = TRUE, list/excluded_mutations = list(/datum/mutation/race))
...
/mob/living/carbon/random_mutate_unique_identity()
	if(!has_dna())
		CRASH("[src] does not have DNA")
...
/mob/living/carbon/random_mutate_unique_features()
	if(!has_dna())
		CRASH("[src] does not have DNA")
...
/mob/living/carbon/clean_dna()
	if(!has_dna())
		CRASH("[src] does not have DNA")
	dna.remove_all_mutations()

/mob/living/carbon/clean_random_mutate(list/candidates, difficulty = 2)
	clean_dna()
	random_mutate(candidates, difficulty)

/mob/living/carbon/human/something_horrible(ignore_humanoidity)
...
/mob/living/carbon/human/something_horrible_mindmelt()
	if(!is_blind())
