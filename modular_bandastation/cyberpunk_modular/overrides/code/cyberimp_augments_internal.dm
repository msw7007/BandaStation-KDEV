// Cyberpunk modular override for code/modules/surgery/organs/internal/cyberimp/augments_internal.dm.
// Core file stays upstream-clean; this file loads late and replaces modified cyberware procs/types.
...
/obj/item/organ/cyberimp/get_overlay(image_layer, obj/item/bodypart/limb)
	. = list()
...
/obj/item/organ/cyberimp/brain/anti_drop/release_items()
	for(var/obj/item/stored_item as anything in stored_items)
...
/obj/item/organ/cyberimp/brain/anti_drop/on_held_item_dropped(obj/item/source, mob/user)
...
/obj/item/organ/cyberimp/brain/anti_stun/on_signal(datum/source, amount)
	if(is_implant_functional() && amount > 0)
		addtimer(CALLBACK(src, PROC_REF(clear_stuns)), stun_cap_amount, TIMER_UNIQUE|TIMER_OVERRIDE)

/obj/item/organ/cyberimp/brain/anti_stun/on_stamcrit(datum/source)
	if(is_implant_functional())
		addtimer(CALLBACK(src, PROC_REF(clear_stuns)), stun_cap_amount, TIMER_UNIQUE|TIMER_OVERRIDE)

/obj/item/organ/cyberimp/brain/anti_stun/clear_stuns()
...
/obj/item/organ/cyberimp/brain/anti_stun/implant_ready()
	if(owner)
		to_chat(owner, span_purple("Your rebooter implant is ready."))

/obj/item/organ/cyberimp/brain/anti_stun/give_stun_buffs(mob/living/give_to = owner)
...
/obj/item/organ/cyberimp/brain/anti_stun/remove_stun_buffs(mob/living/remove_from = owner)
...
/obj/item/organ/cyberimp/brain/anti_stun/reboot()
	organ_flags &= ~ORGAN_FAILING
	implant_ready()
...
/obj/item/organ/cyberimp/brain/surgical_processor/load_surgeries(mob/living/user, obj/design_holder)
